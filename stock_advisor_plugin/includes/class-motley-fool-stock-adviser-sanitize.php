 ...
 * @subpackage Motley Fool Stock Adviser/includes
 */

class Motley_Fool_Stock_Advisor_Sanitize {
	...
	public function clean() {
		 ...
		 */
		do_action( 'mfsa_pre_sanitize', $sanitized );
	...
	public function set_type( $type ) {
		...
		if ( empty( $type ) ) {
			...
			$check = new WP_Error( 'forgot_type', __( 'Specify the data type to sanitize.', 'motley-fool-stock-adviser' ) );

		}

		if ( is_wp_error( $check ) ) {

			wp_die( $check->get_error_message(), __( 'Forgot data type', 'motley-fool-stock-adviser' ) );
